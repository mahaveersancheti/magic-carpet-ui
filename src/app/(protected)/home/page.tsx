...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const red = filteredAndSortedRows.filter(
      (r) => r.warmCallScore >= 0 && r.warmCallScore <= 25,
    ).length;
    const orange = filteredAndSortedRows.filter(
      (r) => r.warmCallScore > 25 && r.warmCallScore <= 50,
    ).length;
    const yellow = filteredAndSortedRows.filter(
      (r) => r.warmCallScore > 50 && r.warmCallScore <= 75,
    ).length;
    const green = filteredAndSortedRows.filter(
      (r) => r.warmCallScore > 75 && r.warmCallScore <= 100,
    ).length;

    return {
      totalLeads: total,
      activeProspects: filteredAndSortedRows.filter(
        (r) => r.status === "PROFILE_EXTRACTED" || r.status === "Pending",
      ).length,
      conversionRate:
        total > 0
          ? (
              (filteredAndSortedRows.filter((r) => r.status === "COMPLETED")
                .length /
                total) *
              100
            ).toFixed(1)
            ...
            {!loading && profiles.length === 0 && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-start gap-3 shadow-sm animate-in fade-in slide-in-from-top-2 duration-300 delay-100">
                ...
                <div className="flex-1 pt-0.5">
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed">
                    You haven't added any leads yet. Start building your
                    pipeline now.
                    <button
                      onClick={() => router.push("/add-lead")}
                      className="cursor-pointer text-blue-600 font-bold hover:underline ml-1"
                    >
                      Add Lead
                    </button>
                  </p>
      ...
      <ConfirmationDialog
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            toast.success(
              `Lead ${deleteConfirmation.name} deleted successfully`,
            );
            setDeleteConfirmation({
              isOpen: false,
              id: null,
