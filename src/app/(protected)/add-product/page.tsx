  ...
  const handleHandleAction = async () => {
    ...
    try {
      ...
      if (productId) {
        ...
        // router.push('/products'); // Don't navigate yet, might need to generate charter
      } else {
        // Create mode
      ...
      if (currentProductId) {
        ...
        try {
          const charterDescription = await dispatch(
            generateCharter(currentProductId),
          ).unwrap();
          if (charterDescription) {
            setProductForm((prev) => ({
              ...prev,
              description: charterDescription,
            }));
            toast.success("Description generated from charter!");
          }
        } catch (charterError) {
          console.error("Charter generation failed:", charterError);
          toast.error("Failed to generate description from charter");
        }
      }

      setLoaderIsSuccess(true);

      // Wait for the success animation to show before closing/navigating
      setTimeout(() => {
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="space-y-2 mt-4 max-h-[200px] overflow-y-auto custom-scrollbar">
                    ...
                    {productDocs.map((file, index) => (
                      <div
                        key={`new-${index}`}
                        className="flex justify-between items-center p-2 bg-blue-50/50 border border-blue-100 rounded-lg"
                      >
                        <div className="flex items-center gap-3">
                    ...
                    {existingDocs.map((path, index) => (
                      <div
                        key={`existing-${index}`}
                        className="flex justify-between items-center p-2 bg-slate-50 border border-slate-100 rounded-lg"
                      >
                        <div className="flex items-center gap-3">
                          <div className="p-1.5 bg-white rounded-md">
